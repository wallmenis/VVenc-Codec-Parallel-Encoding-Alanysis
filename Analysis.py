...
for THREAD in THREAD_NUMBERS:
    ...
    filesizeinthread = []
    for TILE in TILE_NUMBERS:
        filesizeinthread.append(float(os.stat(f"{DATA_OUTPUT}/output-threads{THREAD}-tile{TILE}.266.log").st_size))
    ...
    times.append(timeinthread)
    bitrates.append(bitrateinthread)
    psnrs.append(psnrinthread)
    filesizes.append(filesizeinthread)
...
make_plot(filesizes[0], bitrates[0], "Filesize (bytes)", "Bitrates (Kbps)", "Filesize VS Bitrate", f"{DATA_OUTPUT}/Filesize_vs_bitrate.svg")
